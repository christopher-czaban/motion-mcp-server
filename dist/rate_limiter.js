...
export class RateLimiter {
    ...
    checkAndRecordCall() {
        ...
        const transaction = db.transaction(() => {
            const recentCallsStmt = db.prepare('SELECT COUNT(*) as count FROM api_call_timestamps WHERE timestamp_ms > ?');
            const recentCalls = recentCallsStmt.get(windowStart);
            if (recentCalls.count < MAX_CALLS) {
                ...
                const oldestInWindowStmt = db.prepare('SELECT timestamp_ms FROM api_call_timestamps WHERE timestamp_ms > ? ORDER BY timestamp_ms ASC LIMIT 1');
                const oldestInWindow = oldestInWindowStmt.get(windowStart);
